...
plotInteractions <- function(
  ...
  #' @param colints Custom colors for significant interaction shading. Per default, interactions are plotted using the
  #' transparency parameter alpha of 0.3. However, this can be overridden by providing the alpha parameter together with the color:
  #' col <- rgb(0,0,0,0.2) will use black with an opacity of 0.2.
  ...
  #' @param overwrite.par.settings If TRUE overwrites the default par settings for show.legend.outside
  #' par(mfrow=c(1,2)) by user-provided settings. Only used if show.legend.outside=TRUE. Default: FALSE
  ...
  ##Note: Added AF 31.05. 
  ##Note: Changes to conditional par setting overwriting AF, 9.6.2026
  if (show.legend.outside && overwrite.par.settings==FALSE) {
    par(mfrow=c(1,2))
  }
  ...
  ##AF: Change 1.6.2026: Interactions are plotted behind the line plots to keep
  ##both lines and interactions visible:
  ...
  if (!is.null(d)) {
    ...
    ## 1.6.: AF transparency increased again to 0.3 (from 0.2)
    if (is.null(colints)) {
      cols <- adjustcolor(palette.colors(n = 8, palette = "Okabe-Ito"), 0.3)
      ##Adjust color1 to lower opacity, otherwise it will be too dark
      cols[1] <- adjustcolor(cols[1],0.2)
    } else {
      cols <- adjustcolor(colints, 0.3)
    }
  ...
  ##1.6.2026, AF: After all interactions have been plotted, 
  ##all the lines are plotted in the foreground:
...
boxplotsCapC <- function(annotatedCapCTable, reps = FALSE, col = NULL, show_outliers = TRUE,
    ...
    make_plot <- function(df, cols, sample_names, title, ylab) {
        ...
        ##Note: AF replaced 1.6.26:
        # ggplot(plot_df, aes(x = sample, y = value, fill = sample)) +
        ggplot(plot_df, aes(x = .data$sample, y = .data$value, fill = .data$sample)) +
            geom_boxplot(
                notch = show_notch,
...
plotAggregatePeaks <- function(
    ...
    if (plotwhat == "all") {
        for (type in c("mean", "median")) {
            ...
            ## Added AF 22.08.2026: Supplied ylim overwrites datadriven ylim
            if (is.null(ylimuser)){
        ...
        ## Added AF 26.08.2025: Data-driven ylim:
        ## Added AF 22.08.2026: User-supplied y-lim overwrites data-driven
        if(is.null(ylimuser)){
          ylim <- c(0, max(unlist(Lx)) * 1.1)
        } else {ylim = ylimuser}
...
makeQCplots <- function(ints, folder = NULL, fontsize = 10) {
    ...
    #' @param folder Default: Current folder; Path to the directory in which plots are saved. 
...
## AF: Note 3.9.2026: Changed name of the function from plotSigIntsStats() to plotStats()
...
plotStats <- function(
    ...
    #' 
    #' The summary includes the total number of reads mapped to baits, 
    #' total genes with significant interactions,
    #' total unique and significant interactions, 
    #' % of reads within interactions, as well as
    #' the corresponding distributions per bait.
    #' 
    ...
    #' from Mahara et al. 
    #' @param mode one of 'total' or 'log'; Default: 'total'; if total, plots absolute read count distributions 
    #' per significant interaction, if 'log' plots log2 read counts (pseudocount:1). Only relevant for own data, 
    #' not for the example data.
    #' @examples
    #' # example code:
    #' extdata <- system.file("extdata", package="PostChicago")
    ...
    #' plotStats(L,plotDistribution=FALSE,plotExamples=TRUE)  ##this plots example distributions
    ...
    if (plotDistribution) {
        ...
        ##NOTE AF 03.09.2026: Added the option to plot total reads or log2 read counts
        ##also for lines!
        if(mode=='total'){
        ...
        plot(
           density(log2(dat[[1]]+1)),
           xlab = "Median log2(reads+1) in sig ints per bait",
           lwd = 2,
           main = "Median log2 reads in sig ints per bait distribution",
           col = col[1],
           lty = lty[1]
        )
        ...
        for (i in 2:length(L)) {
            if(mode=='total'){
                ...
                lines(density(log2(dat[[i]]+1)), col = col[i], lwd = 2, lty = lty[i])
            }
        }
        legend(
            "topright",
