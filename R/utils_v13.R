...
baitmap2baited_genes <- function(designDir = "designDir", save = FALSE) {
    ...
    #' @description Searches for the Chicago baitmap file in `designDir` and converts it to a GRanges object.
    #' Only executed if the output file `baited_genes.rds` doesn't exist yet, otherwise loads the existing file.
...
rmap2rmapgr <- function(designDir = "designDir", save = FALSE) {
    ...
    #' @description Searches for the .rmap Chicago file in designDir and converts it to a GenomicRanges object rmapgr.
    #' Only executed if the GenomicRanges object doesn't exist in the design directory yet, otherwise loads the existing file.
...
##NOTE: Changed (AF) 06.09.2026 to make faster!
...
extractValues <- function(downsampledReads, bait, others) {
    ...
    ##downsampledReads as data.table!
    v <- downsampledReads[list(bait,others),N]
    v[is.na(v)] <- 0
    return(v)
}
...
extractValuesScores <- function(downsampledReads, bait, others) {
    ...
    ##downsampledReads as data.table!
    v <- downsampledReads[list(bait,others),score]
    v[is.na(v)] <- 0
    return(v)
}
