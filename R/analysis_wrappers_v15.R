...
makeOneGeneOnePeak <- function(
    ...
    #' @description 
    #' This function takes the interaction table ints (typically containing all significant interactions)
    #' and intersects them with intervals provided in the GRangesList grl. 
    #' 
    #' Each entry in grl receives a unique ID and is assigned only one interaction with each bait with which 
    #' at least one significant interaction could be detected. 
    #' 
    #' Within the resulting OneGeneOnePeak table, start_otherEnd' and 'end_otherEnd' correspond to the start of the first and the 
    #' end of the last restriction fragment that overlap a particular interval. 'otherEndID' corresponds to all overlapped restriction
        ...
        if (!file.exists(file3)) {
            ...
            if (nrow(ov) > 0) {
                ...
                if (nrow(ov) > 0) {
                    ...
                    ## if still different, stop the function
                    ...
                    ##Note: AF Adjustment 13.08.2026:
                    ##reduce() doesn't always work because rmapgr sometimes has gaps between the fragments
                    ##(maybe whenever there is a repeat GATC), then reduce() leads to two intervals instead of one
                    ##and the vectors with baitdata and oedata can have different lengths.
                    ##therefore reduce() was changed to min():max()
                    ...
                    oedata <- do.call("rbind", lapply(as.character(ogop$otherEndID), FUN = function(x) {
                      data.frame(
                        seqnames=as.character(unique(seqnames(rmapgr[rmapgr$id %in% unlist(strsplit(x, split = ","))]))),
                        start=start(rmapgr[rmapgr$id %in% min(as.integer(unlist(strsplit(x, split = ","))))]),
                        end=end(rmapgr[rmapgr$id %in% max(as.integer(unlist(strsplit(x, split = ","))))])
                      )
                    }))
...
quantifyWithinPeaks <- function(data, cdlist, cdlist.reps = NULL, rmapgr, minsize = NULL, maxsize = NULL, file = NULL) {
    ...
    #' @description 
    #' Quantifies reads and scores for interaction tables containing interactions
    #' overlapping multiple restriction fragments, such as oneGenOnePeak or aggregatePeak tables.
    #' 
    #' The function sums up all read and score information within the interaction,
...
##Note: AF adjustment 13.8.2026: 
##remove intDir/ from the name of .rds files!
...
loadGrl <- function(intDir = "intervals") {
    ...
    for (f in ilist) {
        ...
        if (length(grep(".rds$", f)) > 0) {
            ...
            name <- strsplit_string(f, s1=paste0(intDir,'/'), s2 = ".rds")
...
##AF 1.7.2024: Added the option to make sample-specific ints bedfiles
...
makeBedGraphs <- function(
    ...
    #' is created
    #' @param sample_specific_ints Default: FALSE. If TRUE and ints is supplied, creates a bedfile with significant 
    #' interactions for each sample (Chicago score cutoff=5).
    ...
    for (i in 1:length(L)) {
        ...
        ##AF: 1.7.2026: Added sample-specific interactions
        
        intsbedfile <- paste0(outputDir, "/", name, "_id", id, "_", sample, "_ints.bed")
        
        if(!is.null(ints) && sample_specific_ints && !file.exists(intsbedfile)){
          
          intsInId <- ints[ints$baitID == id & ints[,paste0(sample,'_score')]>=5, ]$otherEndID
    ...
    if (!is.null(ints) && !file.exists(intsbedfile)) {
...
getMatrix <- function(
    ...
    if (!file.exists(filename)) {
        ...
        if ("summit_re_id" %in% names(d)) {
            ...
            if (length(grep(",", d$otherEndID)) > 0) {
                ...
                ##NOTE: AF changed 06.09.2026 to fix bug: row duplication by splitting re_id into multiple rows!
                ...
                ids <- strsplit(as.character(d$otherEndID), split=',', fixed=TRUE)
                reps <- lengths(ids)
              
                d <- d[rep(seq_len(nrow(d)),reps), , drop=FALSE]
                d$otherEndID <- unlist(ids, use.names=FALSE)
                d$intID <- paste(d$baitID, d$otherEndID, sep=';')
                rownames(d) <- NULL
        ...
        if (length(d$intID) != length(unique(d$intID))) {
           ##NOTE: AF changed 06.09.2026 to fix bug: row duplication 
            ...
            d <- d[!duplicated(d$intID), , drop = FALSE]
        }
        ...
        ##NOTE: AF (6.9.2026): Added step 1.b - indexing of samples to speed up the process
        
        ## step1b: Index each sample
        message("indexing samples for fast lookup...")
        
        for (i in 1:length(Ldownsampled)){
            Ldownsampled[[i]] <- data.table::as.data.table(Ldownsampled[[i]])
            # if (data.table::anyDuplicated(Ldownsampled[[i]], by = c('baitID', 'otherEndID'))){
            #     warning("duplicate baitID/otherEndID pairs found; collapsing by sum")
            #     Ldownsampled[[i]] <- Ldownsampled[[i]][
            #       , lapply(.SD, sum),
            #       by = list(baitID, otherEndID),
            #       .SDcols = intersect(c("N", "score"), names(Ldownsampled[[i]]))
            #     ]
            # }
            if (anyDuplicated(
                paste(Ldownsampled[[i]]$baitID, Ldownsampled[[i]]$otherEndID, sep=';'), 
                )) {
              stop("duplicate baitID/otherEndID pairs in sample", names(Ldownsampled)[i],
                   "; getMatrix() expects one row per fragment pair")
            }
              data.table::setkey(Ldownsampled[[i]], baitID, otherEndID)
        }
    ...
    ## step7: normalize matrices to the central frag of the normsam matrix, 
        ...
        if (!file.exists(filename2)) {
            ...
            for (i in 1:length(Lm)) {
                for (j in 1:nrow(Lm[[i]])) {
                  ...
                  ##Note: AF adjustment 13.08.2026, because this led to weird artefacts if normsam had 0 and the 
                  ##other sample had high read counts, so that normalizations led to different qualitative results in the plots.
