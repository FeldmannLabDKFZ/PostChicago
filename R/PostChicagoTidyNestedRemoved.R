...
calculateUnderOnePeak <- function(x, what, m, grl, i, zoom, d, a) {
  ...
  ax = a[GenomicRanges::seqnames(a) == d[x, paste0("chr_", names(grl[i]))] & start(a) == d[x, paste0("start_", names(grl[i]))] &
...
boxplotsCC <- function(grl=NULL, resfolder, infolder = '', l = NULL, col=NULL) {
...
makeQCplot <- function(ints, folder = NULL, plot = FALSE) {
  ...
  if (plot) {
    plt <- pheatmap::pheatmap(mraw, show_rownames = FALSE, main = "log2(raw_reads+1)",border_color=NA)
    show(plt)
    ggplot2::ggsave(filename = paste0(folder, "/pheatmap_rawReads_byInt.png"), plot = plt)
    plt <- pheatmap::pheatmap(mnorm, show_rownames = FALSE, main = "log2(downsampled_reads+1)",border_color=NA)
    show(plt)
    ggplot2::ggsave(filename = paste0(folder, "/pheatmap_normReads_byInt.png"), plot = plt)
  } else {
    pheatmap::pheatmap(mraw, show_rownames = FALSE, filename = paste0(folder, "/pheatmap_rawReads_byInt.png"), main = "log2(raw_reads+1)"
                      ,border_color=NA)
    pheatmap::pheatmap(mnorm, show_rownames = FALSE, filename = paste0(folder, "/pheatmap_normReads_byInt.png"),
  ...
  if (plot) {
    plt <- pheatmap::pheatmap(msc, show_rownames = FALSE, main = "asinh(scores)",border_color=NA)
    show(plt)
    ggplot2::ggsave(filename = paste0(folder, "/pheatmap_Scores_byInt.png"), plot = plt)
  } else {
    pheatmap::pheatmap(msc, show_rownames = FALSE, filename = paste0(folder, "/pheatmap_Scores_byInt.png"), 
...
plotSigIntsStats <- function(L,col=NULL,lty=NULL,filename=NULL,plotDistribution=TRUE, plotExamples=FALSE) {
  ...
  #' @param plotExamples if TRUE, example distributions will be plotted based on the data from Mahara et al. Default: FALSE
  #' @examples 
  #' # example code:
  #' extdata <- system.file("extdata", package="PostChicago")
  ...
  if (plotExamples) {
    ...
    legend('topright',cex=0.75,bty='n',col=col[1:length(dat)],legend = paste0(names(dat),',median=',round(sapply(dat, FUN='median'),0)),lwd=2,lty=lty)
