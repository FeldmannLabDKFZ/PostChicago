...
utils::globalVariables(c("grl", "resfolder", "m", "d", "i", "a", "zoom",
                         ...
                         "troubleshooting", "cd", "bg", "scalingL", "id", "bait"))
...
getnormbed <- function(id, data, samplename, L, same.chromosome = FALSE, p = 0, rmapgr, sk=NULL) {
  ...
  GenomicRanges::values(bed) = cbind(as.data.frame(GenomicRanges::values(bed)), as.data.frame(a)[, grep("N", names(a))])
...
bed2gr <- function(bed) {
  #' @title Convert Bed to Genomic Ranges
  #' @description Converts a table in BED3 format to a GenomicRanges object
  #' @param bed bedfile with three columns: chr, start, end
  #' @return GenomicRanges object
  #' @export
...
baitmap2baited_genes <- function(designDir = "designDir", save = FALSE) {
  ...
  #' @description Searches for baitmap file in designDir and converts to a GRanges object.
...
gr2bed <- function(gr) {
  ...
  #' @description Changes GRanges Object to 3 column Bed
...
annotateIntsRepscores <- function(L, ints) {
  #' @title Annotate Replicate Score Interactions
...
makeIntsTable <- function(L, baited_genes, repscores = FALSE, LL = NULL, ngroups = 1, scorecut = 5, readcut = 1,
  ...
  if (length(grep("genename", names(values(baited_genes)))) < 1) {
    stop("baited genes lacks genenames column")
  }
  if (length(grep("re_id", names(values(baited_genes)))) < 1) {
    stop("baited genes lacks re_id column")
  }
  if (length("id" %in% names(values(baited_genes))) < 1) {
...
## TO DO: Check this function that it does not get two entries that are not distinguishable in the output!
...
boxplotsCC <- function(grl, resfolder, infolder = '', l = NULL, col=NULL) {
  #' @title Plot Boxplots from oneGeneonePeak data
  #' @description
  #' Plots scores and reads of each experiment
  #' @param grl GRangesList containing features by which ints should be plotted
  #' @param resfolder folder where matrices are saved
  #' @param infolder Folder
  #' @param l Optional, list with readsScoresInPeak tables from which to make plots. Default: No list supplied, then the information from grl is used for the plots.
...
loadGrl <- function(intDir = "intervals") {
  ...
  #' @param intDir folder containing all interval .rds files, Default:
